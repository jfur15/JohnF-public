...
import mock
from tests.plugins.ReqTracer import requirements, story # pylint: disable=unused-import
...
class TestQuestionDeterminingAnswers(TestCase):
    ...
    def test_question_keywords(self):
        self.assertEqual(
            mainAsk.ask("What type triangle is 1 1 1?"), "equilateral")

        self.assertNotEqual(
            mainAsk.ask("What triangle is 1 1 1?"), "equilateral")

    @requirements(['#0012'])
    def test_question_function_input(self):
        self.assertEqual(
            mainAsk.ask("What type of triangle is 1 2 1?"), "isosceles")


    @requirements(['#0013'])
    def test_question_function_answer(self):
        self.assertEqual(
            mainAsk.ask("What type of quadrilateral is 1 2 1 2?"), "rectangle")
...
class TestQuestionProvidingAnswers(TestCase):
    ...
    def test_question_function_teaching(self):
        ...
        mainAsk.teach(funct)
        self.assertEqual(
            mainAsk.ask("What is the factorial of 4?"), math.factorial(4))

    @requirements(['#0017'])
    def test_question_teaching_none(self):
        mainAsk.ask("Not a question")
        self.assertEqual(
            mainAsk.teach("No it isn't"), "Please ask a question first")

    @requirements(['#0018'])
    def test_question_teaching_refuse(self):
        self.assertEqual(
            mainAsk.ask("What type of triangle is 1 1 1?"), "equilateral")
...
class JobStories(TestCase):
    ...
    @story(['When I ask "What is the n digit of fibonacci" ' +
            'I want to receive the answer so I don\'t have to' +
    ...
    @story(['When I say "Open the door hal", I want the application to ' +
            'say "I\'m afraid I can\'t do that (user name) so I know that' +
            ' is not an option'])
    def test_open_door_user_job_story(self):
        name = "Dave"
        mainAsk.setuser(name)
        self.assertEqual(
            mainAsk.ask("Open the door hal"),
            "I\'m afraid I can\'t do that "+ name)

    @story(['When I ask "Please clear memory" I was the application to clear ' +
            'user set questions and answers so I can reset the application'])
    ...
    @story(['When I ask "Convert <number> <units> to <units>" ' +
            'I want to receive the converted value and units so I can ' +
            'know the answer.', 'When I ask for a numberic conversion I want' +
            ' at least 10 different units I can convert from/to'])
    def test_converted_value_job_story(self):
        self.assertEqual(mainAsk.ask(
            "Convert 10 pounds to ounces"), '160 ounces')
        self.assertEqual(mainAsk.ask(
            "Convert 20 ounces to pounds"), '1.25 pounds')
        ...
        self.assertEqual(mainAsk.ask(
            "Convert 100 centimeters to meters"), '1 meters')
        self.assertEqual(mainAsk.ask(
            "Convert 10 meters to centimeters"), '1000 centimeters')
        self.assertEqual(mainAsk.ask(
            "Convert 500 yards to miles"), '0.284 miles')
        self.assertEqual(mainAsk.ask(
            "Convert 10 miles to yards"), '17600 yards')
        self.assertEqual(mainAsk.ask(
            "Convert 10 grams to kilograms"), '0.1 kilograms')
        self.assertEqual(mainAsk.ask(
            "Convert 10 kilograms to grams"), '1000 grams')
        self.assertEqual(mainAsk.ask(
            "Convert 50 inches to grams"), "Invalid conversion units")

    @story(['When I ask "What is the square root of x" I want to recieve ' +
    ...
    @story(['When I ask "When is the next leap year" I want to receive the ' +
            'answer of the date'])
    def test_leap_year_job_story(self):
        self.assertEqual(mainAsk.ask("What is the next leap year?"), "2020")

    @story(['When I ask "What is my username" I want to receieve an answer' +
            ' or an error'])
    ...
    @story(['When I ask "What is the cube root of x", I want to receive the' +
    ...
    @story(['When I ask "What do ____ measure", I want to know what the' +
            ' unit measures'])
    def test_measurement_job_story(self):
        self.assertEqual(
            mainAsk.ask(
                "What unit do ounces measure?"), "ounces measure weight")

        self.assertEqual(
            mainAsk.ask("What unit do feet measure?"), "feet measure length")
...
class TestInherentCoverage(TestCase):
    ...
    def test_too_many_args(self):
        self.assertRaises(
            Exception, mainAsk.ask, "What is the square root of 10 and 20?")
    ...
    def test_no_last_question(self):
        mainAsk.ask("Please clear memory")
        self.assertEqual(
            mainAsk.correct("Not an answer"), "Please ask a question first")
...
class GitCoverage(TestCase):
    ...
    def test_is_in_repo(self, m):
        ...
        self.assertEqual(
            mainAsk.ask("Is the " + os.getcwd() + \
                "/test/main_test.py in the repo?"), "Yes")

    @mock.patch('subprocess.Popen')
    def test_is_untracked_in_repo(self, m):
        process_mock = mock.Mock()
        attrs = {'communicate.side_effect' : [('', ''), ('', ''), \
                (os.getcwd() + '/test/main_test.pyc', ''), ('', '')]}
        process_mock.configure_mock(**attrs)
        m.return_value = process_mock

        self.assertEqual(
            mainAsk.ask("Is the " + os.getcwd() + \
                "/test/main_test.pyc in the repo?"), "No")
    ...
    def test_does_not_exist(self, m):
        ...
        self.assertEqual(
            mainAsk.ask("Is the /nonexistent.py in the repo?"), "No")
    ...
    def test_file_path_status_up_to_date(self, m):
        ...
        self.assertEqual(
            mainAsk.ask("What is the status of " + os.getcwd() + \
                filepath + "?"), actualfile + " is up to date")
    ...
    def test_file_path_status_dirty_repo(self, m):
        ...
        process_mock = mock.Mock()
        attrs = {'communicate.side_effect' : [("", ''), ('', ''), ('', ''), \
                (os.getcwd() + 'test/main_test.py', ''), ('', ''), \
                (os.getcwd() + 'test/main_test.py', ''), ('', ''), ('', '')]}
        process_mock.configure_mock(**attrs)
        m.return_value = process_mock

        self.assertEqual(mainAsk.ask("What is the status of " + os.getcwd() + \
                        filepath + "?"), actualfile + " is a dirty repo")
    ...
    def test_file_path_status_modified_locally(self, m):
        ...
        actualfile = "main_test.py"
        process_mock = mock.Mock()
        attrs = {'communicate.side_effect' : [(os.getcwd() + \
                "/test/main_test.py", ''), ('', ''), ('', ''), ('', ''), \
                ('', ''), ('', ''), ('', ''), ('', '')]}
        process_mock.configure_mock(**attrs)
        m.return_value = process_mock
        ...
        self.assertEqual(
            mainAsk.ask(
                "What is the status of " + os.getcwd() + filepath + "?"), \
                    actualfile + " has been modified locally")

    @mock.patch('subprocess.Popen')
    def test_file_path_deal_with(self, m):
        filepath = "/test/main_test.py"
        actualfile = "main_test.py"
        fakestats = "6f697e8cb4cd1fakehash2e44813bf8ea9f65dcb, " + \
                    "Wed Jan 24 08:00:00, jfur15"
        process_mock = mock.Mock()
        ...
        self.assertEqual(
            mainAsk.ask(
                "What is the deal with " +  os.getcwd() + filepath + "?"), \
                fakestats)
    ...
    def test_file_path_repo_branch(self, m):
        ...
        self.assertEqual(mainAsk.ask("What branch is " +  os.getcwd() + \
                        filepath + "?"), "branchname1")
    ...
    def test_file_path_repo_url(self, m):
        ...
        actualfile = "main_test.py"
        process_mock = mock.Mock()
        attrs = {'communicate.side_effect' : [("https://github.com/" + \
                "OregonTech/repo1", '')]}
        process_mock.configure_mock(**attrs)
        m.return_value = process_mock

        self.assertEqual(mainAsk.ask("Where did " +  os.getcwd() + filepath + \
                        " come from?"), "https://github.com/OregonTech/repo1")
